          ...
          {apps.map(app => {
            ...
            return (
              <div
                ...
                className={`relative rounded-xl border-2 text-center transition-all cursor-pointer group overflow-hidden
                  ${!app.is_active ? 'opacity-60 cursor-not-allowed' : ''}
                  ${isSelected ? 'ring-2 ring-offset-2 ring-offset-background shadow-lg scale-[1.02]' : 'hover:shadow-lg hover:scale-[1.01]'}`}
                style={{
                  borderColor: isSelected ? app.brand_color : 'transparent',
                  ringColor: app.brand_color,
                }}
              >
                {/* Full-color card background */}
                <div
                  className="p-5"
                ...
                >
                  ...
                  <div className="absolute top-2 left-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10" onClick={e => e.stopPropagation()}>
                    ...
                    <button
                      ...
                      className="w-6 h-6 rounded-md bg-black/20 backdrop-blur-sm flex items-center justify-center hover:bg-red-500/80 transition-colors"
                      title="حذف"
                      style={{ color: app.text_color }}
                    >
                      <Trash2 size={11} />
                  ...
                  <div
                    ...
                    style={{ backgroundColor: 'rgba(255,255,255,0.25)', color: app.text_color }}
                  >
                    {app.name.charAt(0)}
                  </div>
                  ...
                  {app.name_en && (
                    <p className="text-xs mt-0.5 opacity-75" style={{ color: app.text_color }}>{app.name_en}</p>
                  )}
                  <p className="text-3xl font-bold mt-2" style={{ color: app.text_color }}>
                    {app.employeeCount}
                  </p>
                  <p className="text-xs opacity-75" style={{ color: app.text_color }}>مندوب</p>

                  {!app.is_active && (
                    <div className="mt-2">
                      <span className="text-[10px] bg-black/20 px-2 py-0.5 rounded-full" style={{ color: app.text_color }}>
                        معطّل
                      </span>
                    </div>
                  )}
